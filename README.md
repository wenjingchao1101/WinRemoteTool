...
启动本机 Agent（默认监听 `0.0.0.0:27889`，可从局域网访问）：

```powershell
python .\winauto.py agent
```

只监听本机回环地址时使用 `--host 127.0.0.1`。

`agent` 启动是幂等的：端口空闲时创建新的监听；如果同一端口已经是 WinAuto Agent，会完成握手和 `ping` 验证，提示 `already listening` 并正常返回；如果端口被其他程序占用，则拒绝启动并明确报错。

监听非回环地址（默认 `0.0.0.0`，或局域网 IP）时，Agent 还会检查名为 `WinAuto-Agent-TCP-端口号` 的持久入站防火墙规则。规则不存在时自动新增，存在但被禁用或配置错误时自动修复，允许所有网络配置文件访问对应 TCP 端口。首次新增或修复规则需要在“以管理员身份运行”的终端中启动 Agent。监听 `127.0.0.1` 时不会修改防火墙。
...
- 默认绑定 `0.0.0.0`，局域网可连接；需要仅本机访问时使用 `--host 127.0.0.1`。
